package com.wxsoft.telereciver.vc;
...
import com.wxsoft.telereciver.vc.service.TupEventMgr;
import com.wxsoft.telereciver.vc.service.call.data.VideoCaps;
...
public final class LayoutUtil
{
    ...
    private void initialize(int screenWidthPx, int height, float density)
    {
        ...
        TUPLogUtil.i(TAG, "Initialize Screen info density = " + density);
                ...
                - WIDTH_REFERENCE_STANDARD * density) < 0.0000001)
        {
            ...
            dpiScale = density;
            TUPLogUtil.i(TAG, "Layout for normal scale screen such as MediaPad S10");
        }
        else
        {
            setScaleMultiple(1);
            dpiScale = 1;
            TUPLogUtil.i(TAG, "Layout for PX scale screen ");
        }

        if (screenWidthPx / density < 900)
        {
            TUPLogUtil.i(TAG, "Layout for mobile screen");
        }
        else
        {
            TUPLogUtil.i(TAG, "Layout for pad screen");
        }
        ...
        vto.addOnGlobalLayoutListener(new OnGlobalLayoutListener()
        {
            ...
            public void onGlobalLayout()
            {
                ...
                if (view.getLineCount() > lineCount)
                {
                    ...
                    {
                        TUPLogUtil.i(TAG, "text size to small.");
                    }
                    view.setText(text);
                }
            }
    ...
    public void initialize()
    {
        ...
        WindowManager wm = (WindowManager) TupEventMgr.
                getTupContext().getSystemService(Context.WINDOW_SERVICE);
        ...
        TUPLogUtil.i(TAG, metric + ",densityDpi:" + densityDpi);
        if (height > width)
        {
            int temp = height;
            height = width;
    ...
    {
        DisplayMetrics metric = new DisplayMetrics();
        WindowManager wm = (WindowManager) TupEventMgr.getTupContext().
        ...
        TUPLogUtil.i(TAG, "screen size: " + size);
